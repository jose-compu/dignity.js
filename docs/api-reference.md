<!-- Generated by scripts/generate-api-reference.js — do not edit by hand --># dignity.js API reference (v1.0.0)

The scalable data layer of the decentralized browser application ecosystem — P2P object replication with CQRS read replicas, Dignity Apps, and verification policies.
...
- **`connectToPeer(peerId, options)`**
  - open PeerJS data channel to peer id
- **`getConnectionStats()`**
  - Returns: { openCount, peerIds }
- **`ensureConnectedToPeers(peerIds, options)`**
  - connect to many peers before broadcast
- **`joinDiscovery(scope, options)`**
  - scoped presence; options.bootstrapPeerIds connects before announce; advertises verification metadata when registered
- **`leaveDiscovery(scope)`**
  - stop heartbeat and remove local presence from scope
- **`listPeers(scope)`**
  - list presence entries in a discovery scope
- **`announcePresence(scope, options)`**
  - manual presence heartbeat for a joined scope
- **`broadcastMessage(scope, type, payload, options)`**
  - custom app messages; options.connectToPeers
- **`sendDirectMessage(targetId, type, payload, options)`**
  - encrypted direct message to targetId
- **`registerPeerPublicKey(peerId, bundle, options)`**
  - trust peer signing/encryption keys with optional generation
- **`trustPeerPublicKey(peerId, bundle)`**
  - register trusted keys without generation metadata
- **`getPublicKey()`**
  - returns this node's public key bundle
- **`unbanPeer(peerId)`**
  - clear manual or automatic peer ban
- **`getBanInfo(peerId)`**
  - Returns: ban expiry and reason for peerId, or null
- **`banPeer(peerId, durationMs)`**
  - ban peer for durationMs (default peerBanDurationMs); emits peerbanned

### Identity

- **`getPeerIdentityGeneration(peerId)`**
  - trusted identity generation for peerId
- **`getPeerIdentityState(peerId)`**
  - public key + generation state for peerId
- **`adoptDerivedIdentityKeyPair(keyPair)`**
  - install credential-derived keys locally
- **`deriveAndAdoptIdentity({ username, password, ... })`**
  - derive keys from username/password and adopt
- **`broadcastIdentityRotation(options)`**
  - broadcast signed identity:rotate to peers
- **`broadcastColdRecoveryEnrollment(options)`**
  - broadcast cold-recovery enrollment to peers (low-level; pair with enrollColdRecoveryPassword)
- **`enrollAndBroadcastColdRecovery(options)`**
  - convenience: enroll cold recovery key and announce in one call
- **`revokeAndRotateDerivedIdentity(options)`**
  - compromise recovery rotation helper
- **`rotateDerivedIdentityPassword(options)`**
  - password-change rotation helper
- **`applyPeerIdentityRotation(message)`**
  - apply remote identity:rotate message
- **`applyPeerColdRecoveryEnrollment(message)`**
  - apply remote cold-recovery enrollment
...
Package version: **1.0.0** (spec 1.0.0).
